...
const RAW_LINES: Omit<LineDef,'color'>[] = [
  ...
  // ФИОЛЕТОВАЯ МАГИСТРАЛЬ: Новосибирск → Волгоград (через Тольятти / Саратов)
  ...
  // Дубли веток Элисты: синим цветом
...
const COLOR_OVERRIDES: Record<string,string> = {
  ...
  'VLG-ELI-CAUC-PURPLE': '#7c3aed',// Волгоград → Элиста → Невинномысск → Минеральные Воды → Нальчик → Владикавказ
  'VLG-ELI-GRZ-MAH': '#7c3aed',    // Элиста → Будённовск → Грозный → Махачкала (фиолетовый)
  'VLG-ELI-AST-MAH': '#7c3aed',    // Элиста → Астрахань → Махачкала   (фиолетовый)
  'TLT-VLG-PURPLE': '#7c3aed',
...
export default function MetroBranches(){
  ...
  const selfTest = useMemo(()=>{
    ...
    expectPath('NSK-MSK-B','Новосибирск','Москва', 5);    expectPath('MSK-RST','Москва','Владикавказ', 5);
